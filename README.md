...
To compare measured runtimes against theoretical growth rates, I plotted execution times for both implementations alongside $O(n log n)$ and $O(n^2)$ trendlines. These plots give a visual sense of how the algorithms scale as input size increases. For both the Scikit-learn and custom CART implementations, the measured data follows a path that could reasonably match either $O(n log n)$ or $O(n^2)$ over parts of the range, especially at larger input sizes.

![Diabetes custom cart trend](images/custom_trend.png)

The measured runtimes (blue) fall between the $O(n^2)$ (orange) and $O(n log n)$ (green) curves, following $O(n log n)$ more closely in the midrange but converging toward $O(n^2)$ at higher $n$. This suggests the growth rate is not purely quadratic or logarithmic-linear, likely due to implementation-specific overhead in functions like `best_split`.

![Diabetes scikitlearn trend](images/skilearn_trend.png)

The measured runtimes (blue) also track between $O(n log n)$ (green) and $O(n^2)$ (orange), leaning more toward $O(n log n)$ for most input sizes but showing slight deviations as $n$ increases. While the scaling efficiency is strong, the results do not perfectly match a single complexity class, likely due to real-world optimizations and dataset characteristics.

When plotting the theoretical $O(n log n)$ and $O(n^2)$ trendlines, the constant factor was calculated using the runtime at n = 100,000. This means the curves are anchored to match performance at that single point, which can skew their fit for smaller input sizes. As a result, the trendlines may not perfectly represent scaling behavior across the full range of $n$, especially if runtime growth is affected by fixed overhead or other factors not captured in the model.


### Ratio Table

However, visual trendlines can be misleading, as both curves can look similar depending on the scale. To get a clearer answer, I created ratio tables by dividing the measured time by $n^2$ and by $n log n$ for each dataset. If the ratio stays relatively constant for one complexity, it suggests that complexity is the better fit. For Scikit-learn, the (time / n log n) ratio stayed more stable than the (time / n^2) ratio, which points to growth closer to $O(n log n)$. The custom CART ratios were also more consistent for $O(n log n)$, despite having much higher runtimes due to the unoptimized `best_split` function scanning every possible threshold.
...
In this ratio table for the custom CART implementation, it shows a similar trend compared to the Scikit-Learn table. The (time / $(n log n)$ column stays relatively flat across increasing input sizes, which suggests the runtime is growing more closely with $n log n$ complexity. The (time / $( n^2)$) column decreases steadily, meaning the performance does not match the growth of quadratic complexity. You can also see the runtime in seconds which supports the observation that the exhaustive threshold scanning in the `best_split` function introduces significant overhead, making the algorithm scale worse on larger datasets.
...
This project focused on implementing and analyzing the CART Decision Tree algorithm, a foundational supervised machine learning method used for both classification and regression tasks. I chose this topic because of its widespread use in machine learning and scientific research, as well as my interest in understanding its inner workings. My goal was to explore how decision trees operate internally,specifically, how they use impurity measures to determine optimal splits and to evaluate their performance across different datasets. To do this, I implemented two versions of the model in Python: one using the scikit-learn library, which provided a clear and accessible baseline, and another custom implementation of the CART algorithm built from scratch. I then tested both models on three Kaggle datasets: mushroom, iris, and diabetes. They varied in sample size and feature complexity to compare their behavior and performance.

For smaller input sizes (n ≤ 8,000), both implementations showed relatively modest runtimes, though my custom CART was noticeably slower, especially on datasets with more features and complex decision boundaries. This difference became much more pronounced in the large-n experiments (n up to 100,000), where scikit-learn maintained sub-second runtimes for most cases, while my custom implementation scaled poorly reaching over 50 seconds for the largest diabetes dataset. The main bottleneck appears to be in my `best_split` function, which exhaustively scanned all thresholds for every feature at each node. While correct in functionality, this approach has high computational overhead compared to scikit-learn’s optimized methods.

Although decision tree training is theoretically $O(n log n)$, my results suggest mixed scaling behavior. The ratio tables showed relatively stable values for time/(n log n) in smaller and mid-sized datasets, which points toward near $O(n log n)$ growth in that range. However, in the large-n experiments, the runtime curves shifted toward a more quadratic like pattern, likely caused by the cumulative effect of threshold scanning overhead in my implementation. Additionally, the theoretical curves in the trendline graphs were anchored using the runtime at n = 100,000 to calculate constant factors, which can skew how well they fit smaller input sizes making the exact complexity classification less certain.
