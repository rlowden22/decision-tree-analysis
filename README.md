...
In this report, I will explore and analyze decision tree algorithms, a broad class of supervised learning methods used in machine learning. Decision trees represent models as hierarchical tree structures, splitting data into smaller subsets based on measures such as information gain or Gini impurity. They are widely applied to both classification and regression tasks due to their simplicity, interpretability, and ability to handle both categorical and numerical data[^3]. A decision tree consists of a root node, branches, internal decision nodes, and leaf nodes (terminal nodes). The root node has no incoming branches, only outgoing branches leading to decision nodes, which evaluate conditions to split the data into increasingly homogeneous subsets until a leaf node is reached[^1].


![Decision Tree](./images/basic%20Decision%20tree.png)

Classification and Regression Trees (CART) are a specific implementation of decision tree algorithms designed to handle both classification and regression problems. In classification tasks, CART predicts categorical labels, such as determining whether a mushroom is edible or poisonous based on features like color, size, and odor[^3]. In regression tasks, it predicts continuous values, such as estimating house prices from attributes like square footage and location[^4]. The algorithm recursively selects the most informative features at each node, using criteria such as Gini impurity for classification or variance reduction for regression[^5]. One of CART’s key advantages is interpretability—its structure allows us to trace each decision path leading to a final prediction—making it valuable in applications ranging from medical diagnosis to economic forecasting and weather prediction[^6].

This report provides a focused examination of decision tree algorithms, covering their background, theoretical foundations, implementation, and empirical performance. It begins with a brief history and an overview of real-world applications, followed by an explanation of data-splitting criteria such as information gain and gini impurity, and an analysis of time/space complexities using CART pseudocode. The implementation section compares two approaches, a scikit-learn `DecisionTreeClassifier` and a custom CART algorithm applied to three datasets. An empirical analysis evaluates model growth and accuracy across varying dataset sizes and feature counts. The report concludes with a discussion of challenges, key findings, and recommendations for future work.
